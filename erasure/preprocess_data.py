...
def format_med(sample):
    ...
    incorrect_answer_index = random.choice([i for i in range(4) if i != correct_answer_index])
...
def main():
    ...
    med_dataset = med_dataset.map(template_dataset, remove_columns=list(med_dataset.features))
    # print random sample
    print(med_dataset[randint(0, len(med_dataset))]["text"])

    
    medqa_dataset = load_dataset("bigbio/med_qa", "med_qa_en_bigbio_qa",
                                  split="train")
    
    # remove any columns where "type" is not "multiple_choice"
    print(f"medqa_dataset size: {len(medqa_dataset)}")
    medqa_dataset = medqa_dataset.filter(lambda sample: sample["type"] == "multiple_choice")
    print(f"medqa_dataset size: {len(medqa_dataset)}")
    ...
    # apply prompt template per sample
    medqa_dataset = medqa_dataset.map(partial(template_dataset, dataset_type="medqa"), remove_columns=list(medqa_dataset.features))
    # print random sample
    print(medqa_dataset[randint(0, len(medqa_dataset))]["text"])
    auxiliary_train_dataset = load_dataset("cais/mmlu", "abstract_algebra", split="auxiliary_train")
    # remove any columns where "type" is not "multiple_choice"
    ...
    # dataset size: 15011
    # tokenize and chunk dataset
    print(format_auxiliary_train(auxiliary_train_dataset[randrange(len(auxiliary_train_dataset))]))
